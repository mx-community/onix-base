...
if (!globalThis.db.data.settings[conn.user.jid].jadibotmd) return m.reply(`📍  El comando no esta disponible por el momento.\n- Vuelva pronto mientras tanto se ejecutan nuevos servidores.`)
...
console.log(chalk.bold.magentaBright(`\nError`))
try {
if (options.fromCommand) m?.chat ? await conn.sendMessage(`${path.basename(alanJsPath)}@s.whatsapp.net`, {text : 'Sesion desconocida, eliminelo para evitar errores.' }, { quoted: m || null }) : ""
} catch (error) {
console.error(chalk.bold.yellow(`Error +${path.basename(alanJsPath)}`))
}}
if (reason == 405 || reason == 401) {
console.log(chalk.bold.magentaBright(`\nError`))
try {
if (options.fromCommand) m?.chat ? await conn.sendMessage(`${path.basename(alanJsPath)}@s.whatsapp.net`, {text : 'Se ha perdido la conexión, esto puede ser una desconexión o error.' }, { quoted: m || null }) : ""
} catch (error) {
console.error(chalk.bold.yellow(`Error: +${path.basename(alanJsPath)}`))
}
